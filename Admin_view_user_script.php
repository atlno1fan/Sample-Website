							...
							<div class="items-2">
								<?php
									...
									if (isset($_POST['update_user'])) {
										...
										if (mysqli_num_rows($result) > 0) {
											while($row = mysqli_fetch_array($result)){
												...
												<p>Username</p><input type = 'text' name = 'user_name' value = '{$row['Username']}' pattern = '^[A-Za-z0-9]{2,15}$' title='Please input a valid username'>
												...
												<br><br>
												<button type='submit' name='update_user' class='big_btn'>Confirm Update</button> <br>
												<button name='back' class='big_btn'><a href = 'Admin_view_user.php' class='btn'>Go Back</a></button>
												</form>";
											}
										}
									...
									if (isset($_POST['add_user'])) {
												...
												<form action='Additional_files/Admin_view_script.php' method='post'>
												<p>Add New User</p>
												<p>Username</p><input type = 'text' name = 'user_name' pattern = '^[A-Za-z0-9]{2,15}$' title='Please input a valid username' value = '' placeholder ='Name' required>
												<br> 
												...
												<br><br>
												<button type='submit' name='add_user' class='big_btn'>Add User</button> <br>
												<button name='back' class='big_btn'><a href = 'Admin_view_user.php' class='btn'>Go Back</a></button>
												</form>";
									}
									...
									if (isset($_POST['delete_user'])){
										...
										if (mysqli_num_rows($result) > 0) {
											while($row = mysqli_fetch_array($result)){
												...
												<input type = 'text' name = 'user_id' value = '{$userID}' hidden>
												<button type='submit' name='delete_user' class='big_btn'>Delete User</button> <br>
												<button name='back' class='big_btn'><a href = 'Admin_view_user.php' class='btn'>Go Back</a></button>
												</form>
